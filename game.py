...
def new_position(old_position, move):
    ...
    for x in range(12):
        xx = (x * 3) + 2
        xxx = xx + 3
        wpeace_pos = old_position[xx:xxx]
        # for white
        if wpeace_pos[2] == "8":
            s[xx] = "1"
        xx2 = xx + 36
        xxx2 = xxx + 36
        # for black
        bpeace_pos = old_position[xx2:xxx2]
        if bpeace_pos[2] == "1":
            s[xx2] = "1"
    ...
    new_position = "".join(s)
    # return the new position after modification or recant move(call after move validation)
    return new_position
